...
class DeepEnsembleLearner(BaseLearner):
    """
    A causal learner that uses a deep ensemble of models.
    Each ensemble member is an independent run of DiBS with a single particle.
    """

    def __init__(self, config):
        """
        Initialize the Deep Ensemble learner.
        
        Args:
            config (dict): Configuration containing all necessary parameters.
                          Should have 'n_ensemble_runs' for the learner config.
        """
        super().__init__(config)
        
        # Extract learner-specific parameters
        if 'n_ensemble_runs' not in config:
            raise ValueError("DeepEnsembleLearner requires 'n_ensemble_runs' in config")
    ...
    def evaluate(self, particles, x_ho_obs, x_ho_intrv, mask_ho_intrv, g_true, graph_model, likelihood_model):
        ...
        # We pass some real data (x_ho_obs) to satisfy the constructor, but it won't be used for evaluation directly.
